...
int main()
{
   ...
   glEnable(GL_DEPTH_TEST);  
	...
	std::cout<<"lightShader ID: " << lightShader.ID << std::endl;
    ...
    // load and create a texture     stbi_set_flip_vertically_on_load(1);
    // -------------------------
    ...
    unsigned char *data = stbi_load("../textures/cube.jpeg", &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    while (!glfwWindowShouldClose(window))
    {
		...
		model = glm::translate(glm::mat4(1.0),glm::vec3(3.0,2.0,2.5));
        ...
        model = glm::rotate(glm::mat4(1.0), (float)glfwGetTime() * glm::radians(30.0f), glm::vec3(0.5f, 1.0f, 0.0f));
		view = camera.GetViewMatrix();
		translation = glm::mat4(1.0);
        projection = glm::perspective(glm::radians(45.0f), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
        
        cubeShader.set4fv("objectColor", objectColor);
        cubeShader.set4fv("lightColor", lightColor);
        
        
        // retrieve the matrix uniform locations
        cubeShader.setFloat("u_time", glfwGetTime());
        cubeShader.setFloat("isLightSource", 0.0f);
